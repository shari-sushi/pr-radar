...
チェックボックスとトグルは押した時点で盤面に反映される。保存は「次に開いたときも残すか」の意味。

### PR に付ける印

各カードの右下のチェックボックスで `scope-in` / `scope-out` を切り替える。いまは自分がどこまで見るかの線引き用で、盤面の絞り込みには使っていない。
...
既定の `scope-out` はキーごと消す。持ったままにすると、閉じた PR のぶんが増える一方になるため。トークンと違って秘密ではないので暗号化はしていない。設定は `localStorage` に保存されるので、次回からは開くだけ。トークンは端末の外に出ない（送信先は `https://api.github.com` のみ）。
